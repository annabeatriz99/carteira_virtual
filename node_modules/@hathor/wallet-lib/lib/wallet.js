...
var _helpers2 = _interopRequireDefault(require("./utils/helpers"));
...
var _WebSocketHandler = _interopRequireDefault(require("./WebSocketHandler"));
...
const wallet = {
   ...
   */
  _connection: _WebSocketHandler.default,

  /*
   * Customizable gap limit value
   */
  _gapLimit: _constants.GAP_LIMIT,
   ...
   */
  isWalletMultiSig() {
    return Boolean(_storage.default.getItem('wallet:multisig'));
  },
  ...
  executeGenerateWalletFromXPub(xpubkey, loadHistory, multisig) {
    ...
    if (multisig) {
      const xpub = new _bitcoreLib.HDPublicKey(xpubkey);
      access['multisig'] = Object.assign({
        pubkey: xpub.publicKey.toString('hex')
      }, multisig);
    }
  ...
  executeGenerateWalletFromXPriv(xprivkey, pin, loadHistory, multisig) {
    const xpriv = (0, _bitcoreLib.HDPrivateKey)(xprivkey);
    let initialAccessData;
    let privkey;

    if (xpriv.depth === 0) {
      if (multisig) {
        privkey = xpriv.deriveNonCompliantChild(`${_constants.P2SH_ACCT_PATH}/0`);
      } else {
        privkey = xpriv.deriveNonCompliantChild(`m/44'/${_constants.HATHOR_BIP44_CODE}'/0'/0`);
      }
    ...
    const encryptedData = this.encryptData(privkey.xprivkey, pin); // Save in storage the encrypted private key and the hash of the pin and password
    ...
    if (multisig) {
      const dxpriv = xpriv.deriveNonCompliantChild(_constants.P2SH_ACCT_PATH);
      access['multisig'] = Object.assign({
        pubkey: dxpriv.publicKey.toString('hex')
      }, multisig);
    }
  ...
  executeGenerateWallet(words, passphrase, pin, password, loadHistory, multisig) {
    let code = new _bitcoreMnemonic.default(words);
    let xpriv = code.toHDPrivateKey(passphrase, _network.default.getNetwork());
    let authXpriv = xpriv.deriveNonCompliantChild(_constants.WALLET_SERVICE_AUTH_DERIVATION_PATH);
    let privkey;
    ...
    let encryptedDataWords = this.encryptData(words, password); // Save in storage the encrypted private key and the hash of the pin and password
    ...
    let access = {
      ...
      hashIterations: _constants.HASH_ITERATIONS,
      pbkdf2Hasher: 'sha1',
      // For now we are only using SHA1
      xpubkey: privkey.xpubkey
    };
  ...
  getLastGeneratedIndex() {
    const raw = _storage.default.getItem('wallet:lastGeneratedIndex');
   ...
   */
  getWalletData() {
    return _storage.default.getItem('wallet:data');
  },
   ...
   */
  setWalletData(data) {
    _storage.default.setItem('wallet:data', data);
  },
   ...
   */
  getWalletAccessData() {
    return _storage.default.getItem('wallet:accessData');
  },
  ...
  setWalletAccessData(data) {
    _storage.default.setItem('wallet:accessData', data);
  },
  ...
  getAddressP2PKH(index) {
    const accessData = this.getWalletAccessData();
    const xpub = (0, _bitcoreLib.HDPublicKey)(accessData.xpubkey);
    const newKey = xpub.deriveChild(index);
    return new _bitcoreLib.Address(newKey.publicKey, _network.default.getNetwork());
  },
  ...
  getAddressMultisig(index) {
    ...
    const redeemScript = _wallet2.default.createP2SHRedeemScript(multisigData.pubkeys, multisigData.minSignatures, index);

    return _bitcoreLib.Address.payingTo(_bitcoreLib.Script.fromBuffer(redeemScript), _network.default.getNetwork());
  },
  ...
  loadAddressHistory(startIndex, count, connection = null, store = null) {
    const promise = new Promise((resolve, reject) => {
      ...
      for (var i = startIndex; i < stopIndex; i++) {
        ...
        if (_storage.default.getItem('wallet:address') === null) {
      ...
      if (lastGeneratedIndex < stopIndex - 1) {
        _storage.default.setItem('wallet:lastGeneratedIndex', stopIndex - 1);
      }

      this.setWalletData(dataJson); // Set back to old store because won't use storage in this method anymore

      _storage.default.setStore(oldStore);

      this.getTxHistory(addresses, resolve, reject, connection, store).catch(e => {
        reject(e);
      });
    });
    return promise;
  ...
  async getTxHistory(addresses, resolve, reject, connection = null, store = null) {
    ...
    const addressesChunks = _lodash.default.chunk(addresses, _constants.MAX_ADDRESSES_GET);
    ...
    for (let i = 0; i <= lastChunkIndex; i++) {
      let hasMore = true;
      let firstHash = null;
      let addressesToSearch = addressesChunks[i]; // Subscribe in websocket to the addresses
      ...
      while (hasMore === true) {
        ...
        try {
          response = await _wallet.default.getAddressHistoryForAwait(addressesToSearch, firstHash);
        } catch (e) {
          ...
          retryCount += 1;
          await _helpers2.default.sleep(_constants.LOAD_WALLET_RETRY_SLEEP);
          continue;
        ...
        if (result.success) {
          ...
          if (hasMore || i !== lastChunkIndex) {
            ...
            const allTokens = 'allTokens' in data ? data['allTokens'] : []; // Set back to old store because won't use storage in this method anymore
  ...
  updateAddress(lastSharedAddress, lastSharedIndex) {
    _storage.default.setItem('wallet:address', lastSharedAddress);

    _storage.default.setItem('wallet:lastSharedIndex', lastSharedIndex);
  },
  ...
  encryptData(privateKey, pin, salt) {
    const encrypted = _cryptoJs.default.AES.encrypt(privateKey, pin);
  ...
  oldHashPassword(password) {
    return _cryptoJs.default.SHA256(_cryptoJs.default.SHA256(password));
  },
  ...
  hashPassword(password, salt) {
    if (salt === undefined) {
      salt = _cryptoJs.default.lib.WordArray.random(128 / 8).toString();
    ...
    const key = _cryptoJs.default.PBKDF2(password, salt, {
  ...
  decryptData(data, password) {
    let decrypted = _cryptoJs.default.AES.decrypt(data, password);
  ...
  hashValidation(password, hashKey, saltKey) {
    ...
    if (!(saltKey in accessData)) {
      ...
      accessData['hashIterations'] = _constants.HASH_ITERATIONS;
  ...
  _getChangePasswordAccessData(accessData, oldPassword, newPassword) {
    ...
    const newHash = this.hashPassword(newPassword); // Get and update seed encrypted with PIN

    const decryptedData = this.decryptData(accessData.words, oldPassword);
    const encryptedData = this.encryptData(decryptedData, newPassword); // Create a new object (without mutating the old one) with the updated data
  ...
  _getChangePinAccessData(accessData, oldPin, newPin) {
    // Get new PIN hash
    const newHash = this.hashPassword(newPin); // Get and update data encrypted with PIN

    const decryptedMainKey = this.decryptData(accessData.mainKey, oldPin);
    const encryptedMainKey = this.encryptData(decryptedMainKey, newPin); // Create a new object (without mutating the old one) with the updated data
  ...
  changePinAndPassword({
    ...
    let newAccessData = { ...oldAccessData
    };
  ...
  generateNewAddress(connection = null) {
    ...
    const newAddress = this.generateAddress(newIndex); // Update address data and last generated indexes

    this.updateAddress(newAddress.toString(), newIndex);
    let lastGeneratedIndex = this.getLastGeneratedIndex();

    if (newIndex > lastGeneratedIndex) {
      _storage.default.setItem('wallet:lastGeneratedIndex', newIndex);
    ...
    this.setWalletData(data); // Subscribe in ws to new address updates
  ...
  getAddressToUse(connection = null) {
    const address = this.getCurrentAddress(); // Updating address because the last one was used
   ...
   */
  getCurrentAddress() {
    return _storage.default.getItem('wallet:address');
  },
  ...
  filterUtxos(output, utxoDetails, options) {
    const filterOptions = Object.assign({
      max_utxos: _transaction.default.getMaxInputsConstant(),
      token: _constants.HATHOR_TOKEN_CONFIG.uid,
      filter_address: null,
      amount_smaller_than: Infinity,
      amount_bigger_than: 0,
      maximum_amount: Infinity
    }, options); // Filter by address, if options.filter_address is specified

    const is_address_valid = filterOptions.filter_address === null || filterOptions.filter_address === output.decoded.address; // Filter by maximum_amount (sum of utxos amounts), if options.maximum_amount is specified

    const is_max_amount_valid = filterOptions.maximum_amount >= utxoDetails.total_amount_available + output.value; // Filter more utxos than options.max_utxos (default: transaction.getMaxInputsConstant())

    const is_max_utxos_valid = filterOptions.max_utxos > utxoDetails.utxos.length; // Filter other tokens, if options.token is specified

    const is_token_valid = filterOptions.token === output.token; // Filter by options.amount_smaller_than, if it is specified

    const is_amount_smaller_than_valid = filterOptions.amount_smaller_than >= output.value; // Filter by options.amount_bigger_than, if it is specified
  ...
  canUseUnspentTx(unspentTx, blockHeight) {
    if (unspentTx.decoded.timelock) {
      let currentTimestamp = _date.default.dateToTimestamp(new Date());
   ...
   */
  loaded() {
    return _storage.default.getItem('wallet:accessData') !== null;
  },
   ...
   */
  started() {
    return _storage.default.getItem('wallet:started') !== null;
  },
   ...
   */
  markWalletAsStarted() {
    return _storage.default.setItem('wallet:started', true);
  },
  ...
  setLastUsedIndex(address) {
    ...
    if (data) {
      ...
      if (lastUsedIndex === null || index > parseInt(lastUsedIndex, 10)) {
        _storage.default.setItem('wallet:lastUsedAddress', address);

        _storage.default.setItem('wallet:lastUsedIndex', index);
      }
    }
  },
   ...
   */
  cleanServer() {
    _storage.default.removeItem('wallet:server');
  },
   ...
   */
  cleanLoadedData({
    cleanAccessData = true
  } = {}) {
    if (cleanAccessData) {
      _storage.default.removeItem('wallet:accessData');
    }

    _storage.default.removeItem('wallet:data');

    _storage.default.removeItem('wallet:address');

    _storage.default.removeItem('wallet:lastSharedIndex');

    _storage.default.removeItem('wallet:lastGeneratedIndex');

    _storage.default.removeItem('wallet:lastUsedIndex');

    _storage.default.removeItem('wallet:lastUsedAddress'); // we clean storage, but wallet is still open
  ...
  resetWalletData() {
    ...
    _transaction.default.clearTransactionWeightConstants();

    _transaction.default.clearMaxInputsConstant();

    _transaction.default.clearMaxOutputsConstant();

    _tokens.default.clearDepositPercentage();

    this.clearRewardLockConstant();
    this.clearNetworkBestChainHeight();

    _storage.default.removeItem('wallet:started');

    _storage.default.removeItem('wallet:backup');

    _storage.default.removeItem('wallet:locked');

    _storage.default.removeItem('wallet:tokens');

    _storage.default.removeItem('wallet:sentry');

    _storage.default.removeItem('wallet:type');
  },
  ...
  getInputsFromAmount(historyTransactions, amount, selectedToken) {
      ...
      for (const [index, txout] of tx.outputs.entries()) {
        ...
        if (txout.spent_by === null && txout.token === selectedToken && this.isAddressMine(txout.decoded.address, data)) {
          if (this.canUseUnspentTx(txout, tx.height)) {
            ...
            const utxo = Object.assign({}, txout, {
              tx_id: tx.tx_id,
              index
            });
            utxos.push(utxo);
    ...
    if (selectedUtxo) {
      ...
      ret.inputs.push({
        tx_id: selectedUtxo.tx_id,
        index: selectedUtxo.index,
        token: selectedToken,
        address: selectedUtxo.decoded.address
      });
      ...
      const sortedUtxos = _lodash.default.orderBy(utxos, ['value'], ['desc']);
   ...
   * @inner
   */
  getOutputChange(value, tokenData, options = {
    address: null
  }) {
    const {
      address
    ...
    if (address) {
      if (!_transaction.default.isAddressValid(address)) {
        throw new _errors.AddressError('Change address is invalid.');
      }

      if (!this.isAddressMine(address)) {
        throw new _errors.AddressError('Change address is not from this wallet.');
      }
    }
   ...
   */
  lock() {
    _storage.default.setItem('wallet:locked', true);
  },
   ...
   */
  isLocked() {
    return _storage.default.getItem('wallet:locked') !== null;
  },
   ...
   */
  wasClosed() {
    return _storage.default.getItem('wallet:closed') === true;
  },
   ...
   */
  close() {
    _storage.default.setItem('wallet:closed', true);
  },
   ...
   */
  setWalletAsOpen() {
    _storage.default.setItem('wallet:closed', false);
  },
  ...
  getAuthPrivKey(pin) {
    const accessData = this.getWalletAccessData();
    return (0, _bitcoreLib.HDPrivateKey)(this.decryptData(accessData.authKey, pin));
  },
   ...
   */
  markBackupAsDone() {
    _storage.default.setItem('wallet:backup', true);
  },
   ...
   */
  isBackupDone() {
    return _storage.default.getItem('wallet:backup') !== null;
  },
  ...
  reloadData({
    ...
    this.setWalletAccessData(accessData);
    this.setWalletData(newWalletData); // Load history from new server
  ...
  isAuthorityOutput(output) {
    return _transaction.default.isTokenDataAuthority(output.token_data);
  },
  ...
  changeServer(newServer) {
    _storage.default.setItem('wallet:server', newServer);
  },
  ...
  setDefaultServer(server) {
    _storage.default.setItem('wallet:defaultServer', server);
  },
   ...
   */
  clearDefaultServer() {
    _storage.default.removeItem('wallet:defaultServer');
  },
   ...
   * @inner
   */
  prepareSendTokensData(data, token, chooseInputs, historyTransactions, allTokens, options = {
    changeAddress: null
  }) {
    // Get the data and verify if we need to select the inputs or add a change output
    const {
      changeAddress
    } = options; // First get the amount of outputs
    ...
    if (chooseInputs) {
      ...
      if (newData.inputsAmount > outputsAmount) {
        ...
        try {
          outputChange = this.getOutputChange(newData.inputsAmount - outputsAmount, _tokens.default.getTokenIndex(allTokens, token.uid), {
            address: changeAddress
          });
        } catch (e) {
        ...
        data['outputs'] = _lodash.default.shuffle(data['outputs']);
      }
    } else {
      ...
      for (const input of data.inputs) {
        ...
        if (this.canUseUnspentTx(output, utxo.height)) {
          ...
          return {
            success: false,
            message: `Token: ${token.symbol}. Output [${input.tx_id}, ${input.index}] is locked until ${_date.default.parseTimestamp(output.decoded.timelock)}`
          };
        }
      }
      ...
      if (inputsAmount > outputsAmount) {
        // Need to create change output
        try {
          outputChange = wallet.getOutputChange(inputsAmount - outputsAmount, _tokens.default.getTokenIndex(allTokens, token.uid), {
            address: changeAddress
          });
        } catch (e) {
          if (e instanceof _errors.AddressError) {
  ...
  getLocalStorageIndex(key) {
    let index = _storage.default.getItem(`wallet:${key}`);
  ...
  saveNewHistoryOnStorage(oldHistoryTransactions, oldAllTokens, newHistory, dataJson, connection = null, store = null) {
    ...
    for (const tx of newHistory) {
      ...
      for (const output of tx.outputs) {
        if (output.value > _constants.MAX_OUTPUT_VALUE) {
          throw new _errors.OutputValueError(`Transaction with id ${tx.tx_id} has output value of ${_helpers.default.prettyValue(output.value)}. Maximum value is ${_helpers.default.prettyValue(_constants.MAX_OUTPUT_VALUE)}`);
        }
    ...
    const lastGeneratedIndex = this.getLastGeneratedIndex(); // Set back to old store because won't use storage in this method anymore
  ...
  areInputsMine(txData) {
    ...
    if (_helpers.default.isBlock(txData)) return false;
    const data = this.getWalletData();
    let mine = true;
  ...
  getTokenIndex(token_data) {
    return token_data & _constants.TOKEN_INDEX_MASK;
  },
  ...
  getRewardLockConstant() {
    if (this._rewardSpendMinBlocks === null) {
      throw new _errors.ConstantNotSet('Reward block minimum blocks constant not set');
    }
  ...
  setWalletType(type) {
    ...
    if (!walletTypes.includes(type)) {
      throw new _errors.WalletTypeError('Invalid wallet type');
    }

    _storage.default.setItem('wallet:type', type);
  },
   ...
   */
  isHardwareWallet() {
    return _storage.default.getItem('wallet:type') === 'hardware';
  },
  ...
  xpubFromData(pubkey, chainCode, fingerprint) {
    const hdpubkey = new _bitcoreLib.HDPublicKey({
      network: _network.default.getNetwork(),
      depth: 4,
  ...
  getPublicKey(index) {
    const accessData = this.getWalletAccessData();
    const hdpubkey = (0, _bitcoreLib.HDPublicKey)(accessData.xpubkey);
  ...
  getXPubKeyFromXPrivKey(pin) {
    ...
    const privateKey = (0, _bitcoreLib.HDPrivateKey)(privateKeyStr);
