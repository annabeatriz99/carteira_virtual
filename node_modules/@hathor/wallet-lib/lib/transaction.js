...
const transaction = {
  ...
  signedIntToBytes(number, bytes) {
    ...
    if (bytes === 1) {
      ...
      let long = _long.default.fromNumber(number, false);
  ...
  validateAddress(address, addressBytes) {
    ...
    if (!_network.default.isVersionByteValid(addressBytes[0])) {
      throw new _errors.AddressError(errorMessage);
    }
  ...
  dataToSign(txData) {
    ...
    arr.push(this.intToBytes(txData.version, 2)); // Len tokens
    ...
    arr.push(this.intToBytes(txData.outputs.length, 1)); // Tokens data
    ...
    for (let inputTx of txData.inputs) {
      arr.push((0, _buffer.hexToBuffer)(inputTx.tx_id));
    ...
    if (txData.version === _constants.CREATE_TOKEN_TX_VERSION) {
  ...
  getSignature(index, hash, pin) {
    ...
    const privateKeyStr = _wallet.default.decryptData(encryptedPrivateKey, pin);
  ...
  getDataToSignHash(dataToSign) {
    const hashbuf = _bitcoreLib.crypto.Hash.sha256sha256(dataToSign);
  ...
  calculateTxWeight(txData) {
    ...
    if (txData.parents && txData.parents.length > 2) {
      throw new _errors.MaximumNumberParentsError(`Transaction has ${txData.parents.length} parents and can have at most 2.`);
    } // Parents may not be sent in txData


    const parents = (0, _lodash.get)(txData, 'parents') || [];
    txSize += 64 - 32 * parents.length;
    let sumOutputs = this.getOutputsSum(txData.outputs); // Preventing division by 0 when handling authority methods that have no outputs

    sumOutputs = Math.max(1, sumOutputs); // We need to take into consideration the decimal places because it is inside the amount.
  ...
  completeTx(incompleteTxData) {
    ...
    const newData = Object.assign({
      ...
      timestamp: _date.default.dateToTimestamp(new Date())
    }, incompleteTxData); // Update incompleteTxData.
  ...
  txToBytes(txData) {
    ...
    if (txData.parents) {
      ...
      for (const parent of txData.parents) {
        arr.push((0, _buffer.hexToBuffer)(parent));
      }
    } else {
  ...
  verifyTxData(data) {
    ...
    if (data.inputs.length > maxNumberInputs) {
      throw new _errors.MaximumNumberInputsError(`Transaction has ${data.inputs.length} inputs and can have at most ${maxNumberInputs}.`);
    }

    if (data.outputs.length > maxNumberOutputs) {
      throw new _errors.MaximumNumberOutputsError(`Transaction has ${data.outputs.length} outputs and can have at most ${maxNumberOutputs}.`);
    }
  },
  ...
  sendTransaction(data, pin, options) {
    try {
      ...
      const message = _helpers2.default.handlePrepareDataError(e);
  ...
  sendPreparedTransaction(data) {
    const promise = new Promise((resolve, reject) => {
      ...
      _wallet2.default.sendTokens(txHex, response => {
        if (response.success) {
          resolve(response);
        } else {
          let message = '';

          if (response.return_code === _constants.STRATUM_TIMEOUT_RETURN_CODE) {
  ...
  getTransactionWeightConstants() {
    if (this._weightConstants === null) {
      throw new _errors.ConstantNotSet('Transaction weight constants are not set');
    }
  ...
  serializeTokenInfo(txData) {
    ...
    const nameBytes = _buffer2.default.Buffer.from(txData.name, 'utf8');

    const symbolBytes = _buffer2.default.Buffer.from(txData.symbol, 'utf8');

    const arr = []; // Token info version

    arr.push(this.intToBytes(_constants.TOKEN_INFO_VERSION, 1)); // Token name size

    arr.push(this.intToBytes(nameBytes.length, 1)); // Token name

    arr.push(nameBytes); // Token symbol size

    arr.push(this.intToBytes(symbolBytes.length, 1)); // Token symbol
  ...
  isTokenDataAuthority(tokenData) {
    return (tokenData & _constants.TOKEN_AUTHORITY_MASK) > 0;
  },
  ...
  getMaxInputsConstant() {
    if (this._maxInputsConstant === null) {
      throw new _errors.ConstantNotSet('Maximum number of inputs constants is not set yet');
    }
  ...
  getMaxOutputsConstant() {
    if (this._maxOutputsConstant === null) {
      throw new _errors.ConstantNotSet('Maximum number of outputs constants is not set yet');
    }
