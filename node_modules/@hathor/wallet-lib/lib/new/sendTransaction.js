...
class SendTransaction extends _events.default {
  ...
  constructor({
    ...
    this.fullTxData = null; // Error to be shown in case of an unexpected error when executing push tx

    this.unexpectedPushTxError = _errorMessages.ErrorMessages.UNEXPECTED_PUSH_TX_ERROR; // Stores the setTimeout object to set selected outputs as false
  ...
  prepareTxData() {
    const tokensData = {};
    const HTR_UID = _constants.HATHOR_TOKEN_CONFIG.uid; // PrepareSendTokensData method expects all inputs/outputs for each token
    ...
    const fullTxData = Object.assign({
      tokens
    }, getDefaultData());
    const tokensUids = tokens.map(token => {
      return {
        uid: token
      };
    });
    ...
    for (const tokenUid in tokensData) {
      ...
      const ret = _wallet.default.prepareSendTokensData(partialData, {
        uid: tokenUid
      }, chooseInputs, historyTxs, tokensUids, {
        changeAddress: this.changeAddress
      });

      if (!ret.success) {
        ret.debug = {
          balance: _wallet.default.calculateBalance(Object.values(historyTxs), tokenUid),
          partialData: partialData,
          // this might not be the original `partialData`
          tokenUid,
          ...ret.debug
        };
        throw new _errors.SendTxError(ret.message);
      }
  ...
  prepareTx() {
    ...
    try {
      preparedData = _transaction.default.prepareData(this.fullTxData, this.pin);
      this.transaction = _helpers.default.createTxFromData(preparedData, this.network);
  ...
  prepareTxFrom(signatures) {
    ...
    const keys = _wallet.default.getWalletData().keys;

    for (const [index, input] of this.fullTxData.inputs.entries()) {
      const signature = signatures[index];
      const keyIndex = keys[input.address].index;

      const pubkey = _wallet.default.getPublicKey(keyIndex);

      input['data'] = _transaction.default.createInputData(signature, pubkey);
    ...
    try {
      preparedData = _transaction.default.prepareData(this.fullTxData, null, {
        getSignature: false,
        completeTx: false
      });
      this.transaction = _helpers.default.createTxFromData(preparedData, this.network);
  ...
  updateOutputSelected(selected, store = null) {
    ...
    const allTokens = 'allTokens' in walletData ? walletData.allTokens : []; // Before sending the tx to be mined, we iterate through the inputs and set selected_as_input
