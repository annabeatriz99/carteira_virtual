...
var _wallet2 = _interopRequireDefault(require("../api/wallet"));
...
var _memory_store = _interopRequireDefault(require("../memory_store"));
...
class HathorWallet extends _events.default {
  ...
  constructor({
    ...
    _wallet.default.setConnection(connection);

    this.state = HathorWallet.CLOSED;
    this.serverInfo = null;
    this.xpriv = xpriv;
    this.seed = seed;
    this.xpub = xpub; // tokenUid is optional so we can get the token of the wallet
    ...
    if (store) {
      ...
      this.store = new _memory_store.default();
    }

    _storage.default.setStore(this.store);

    this.onConnectionChangedState = this.onConnectionChangedState.bind(this);
    this.handleWebsocketMsg = this.handleWebsocketMsg.bind(this); // Used to know if the wallet is loading data for the first time
    ...
    this.firstConnection = true; // Debug mode. It is used to include debugging information
    // when a problem occurs.

    this.debug = debug; // The reload is called automatically in the lib when the ws reconnects
    // this callback gives a chance to the apps to run a method before reloading data in the lib

    this.beforeReloadCallback = beforeReloadCallback; // Set to true when stop() method is called

    this.walletStopped = false; // This object stores pre-processed data that helps speed up the return of getBalance and getTxHistory
  ...
  onConnectionChangedState(newState) {
    if (newState === ConnectionState.CONNECTED) {
      ...
      this.setState(HathorWallet.SYNCING); // If it's the first connection we just load the history
      ...
      if (this.firstConnection) {
        ...
        promise = _wallet.default.loadAddressHistory(0, _wallet.default.getGapLimit(), this.conn, this.store);
      } else {
        if (this.beforeReloadCallback) {
          this.beforeReloadCallback();
        }

        promise = _wallet.default.reloadData({
  ...
  getAllSignatures(txHex, pin) {
    ...
    const walletData = _wallet.default.getWalletData();

    const historyTransactions = walletData['historyTransactions'] || {};

    const accessData = _storage.default.getItem('wallet:accessData');

    const privateKeyStr = _wallet.default.decryptData(accessData.mainKey, pin);

    const key = (0, _bitcoreLib.HDPrivateKey)(privateKeyStr);
    const signatures = {};

    for (const {
      index,
      input
    } of tx.inputs.map((input, index) => ({
      index,
      input
    }))) {
      ...
      const address = histTx.outputs[input.index].decoded.address; // get address index

      const addressIndex = _wallet.default.getAddressIndex(address);
      ...
      const privateKey = derivedKey.privateKey; // derive key to address index

      const sig = _bitcoreLib.crypto.ECDSA.sign(hash, privateKey, 'little').set({
    ...
    const p2shSig = new _p2sh_signature.default(accessData.multisig.pubkey, signatures);
  ...
  assemblePartialTransaction(txHex, signatures) {
    ...
    const xpub = (0, _bitcoreLib.HDPublicKey)(accessData.xpubkey); // deserialize P2SHSignature for all signatures

    const p2shSignatures = signatures.map(sig => _p2sh_signature.default.deserialize(sig));
    ...
    for (const {
      ...
      const redeemScript = _wallet3.default.createP2SHRedeemScript(multisigData.pubkeys, multisigData.minSignatures, addressIndex);
      ...
      const inputData = _wallet3.default.getP2SHInputData(sigs, redeemScript);
  ...
  _getAllAddressesRaw() {
    ...
    return _wallet.default.getAllAddresses();
  }
  /**
  ...
  getAllAddresses() {
    ...
    return _wrapAsyncGenerator(function* () {
      _storage.default.setStore(_this.store); // This algorithm is bad at performance
      ...
      const transactionsByAddress = _this.getTransactionsCountByAddress();

      const addresses = _wallet.default.getAllAddresses();
  ...
  getAddressAtIndex(index) {
    ...
    return _wallet.default.getAddressAtIndex(index);
  }
  /**
  ...
  getCurrentAddress({
    ...
    if (markAsUsed) {
      address = _wallet.default.getAddressToUse(this.conn);
    } else {
      address = _wallet.default.getCurrentAddress();
    }

    const index = this.getAddressIndex(address);
    let addressPath;

    if (_wallet.default.isWalletMultiSig()) {
      addressPath = `${_constants.P2SH_ACCT_PATH}/0/${index}`;
    } else {
      addressPath = `m/44'/${_constants.HATHOR_BIP44_CODE}'/0'/0/${index}`;
    }
  ...
  async getBalance(token = null) {
    _storage.default.setStore(this.store); // TODO if token is null we should get the balance for each token I have
    ...
    const balance = uid in balanceByToken ? balanceByToken[uid] : {
      available: 0,
      locked: 0,
      transactions: 0
    };
    return [{
      ...
      tokenAuthorities: {
        unlocked: {
          mint: this.selectAuthorityUtxo(uid, _wallet.default.isMintOutput.bind(_wallet.default)) !== null,
          melt: this.selectAuthorityUtxo(uid, _wallet.default.isMeltOutput.bind(_wallet.default)) !== null
        },
        locked: {
          mint: false,
          melt: false
  ...
  async getTxHistory(options = {}) {
    ...
    const newOptions = Object.assign({
      token_id: _constants.HATHOR_TOKEN_CONFIG.uid,
      count: 15,
      skip: 0
    }, options);
    const {
  ...
  getAddressInfo(address, options = {}) {
    ...
    } = options; // Throws an error if the address does not belong to this wallet
    ...
    const index = this.getAddressIndex(address); // All transactions for this address

    const historyTransactions = Object.values(this.getFullHistory()); // Address information that will be calculated below
    ...
    historyTransactions.forEach(transaction => {
      ...
      transaction.outputs.forEach(output => {
        ...
        const is_authority = _wallet.default.isAuthorityOutput(output);

        if (!is_address_valid || !is_token_valid || is_authority) {
          return;
        }

        const is_spent = output.spent_by !== null; // wallet.canUseUnspentTx handles locking by timelock, by blockHeight and by utxos already being used by this wallet.

        const is_locked = !_wallet.default.canUseUnspentTx(output, transaction.height);
  ...
  getUtxos(options = {}) {
    ...
    for (let i = 0; i < historyTransactions.length; i++) {
      const transaction = historyTransactions[i]; // Voided transactions should be ignored

      if (transaction.is_voided) continue; // Iterate through outputs

      for (let j = 0; j < transaction.outputs.length; j++) {
        const output = transaction.outputs[j];
        const is_unspent = output.spent_by === null; // wallet.canUseUnspentTx handles locking by timelock, by blockHeight and by utxos already being used by this wallet.

        const locked = !_wallet.default.canUseUnspentTx(output, transaction.height);
        ...
        const filters = _wallet.default.filterUtxos(output, utxoDetails, options);

        const is_authority = _wallet.default.isAuthorityOutput(output); // Max amount reached, continue to find a smaller amount
  ...
  prepareConsolidateUtxosData(destinationAddress, options = {}) {
    ...
    for (let i = 0; i < utxoDetails.utxos.length; i++) {
      if (inputs.length === _transaction.default.getMaxInputsConstant()) {
    ...
    const outputs = [{
      ...
      value: total_amount,
      token: options.token || _constants.HATHOR_TOKEN_CONFIG.uid
    }];
    return {
      outputs,
      inputs,
  ...
  async consolidateUtxos(destinationAddress, options = {}) {
    ...
    const {
      ...
      utxos,
      total_amount
    } = this.prepareConsolidateUtxosData(destinationAddress, options);
    ...
    const tx = await this.sendManyOutputsTransaction(outputs, {
      inputs
    });
    return {
  ...
  _getBalanceRaw(tokenUid) {
    ...
    return _wallet.default.calculateBalance(Object.values(historyTransactions), uid);
  }
  /**
  ...
  getFullHistory() {
    ...
    const data = _wallet.default.getWalletData();
  ...
  async preProcessWalletData() {
    ...
    const history = this.getFullHistory();
    const tokensHistory = {}; // iterate through all txs received and map all tokens this wallet has, with
    ...
    for (const tx of Object.values(history)) {
      ...
      const balances = await this.getTxBalance(tx, {
    ...
    for (const tokenUid of Object.keys(tokensHistory)) {
      ...
      totalBalance['transactions'] = transactionCountByToken[tokenUid]; // update token total balance
  ...
  async onTxArrived(tx, isNew) {
    ...
    const tokensBalance = await this.getPreProcessedData('balanceByToken'); // we first get all tokens present in this tx (that belong to the user) and
    // the corresponding balances
  ...
  onNewTx(wsData) {
    ...
    const newTx = wsData.history; // TODO we also have to update some wallet lib data? Lib should do it by itself
    ...
    _wallet.default.updateHistoryData(historyTransactions, allTokens, [newTx], null, walletData, null, this.conn, this.store);
  ...
  async sendManyOutputsTransaction(outputs, options = {}) {
    ...
    if (!pin) {
      return {
        success: false,
        message: ERROR_MESSAGE_PIN_REQUIRED,
        error: ERROR_CODE_PIN_REQUIRED
      };
    }
  ...
  start(optionsParams = {}) {
    ...
    _storage.default.setItem('wallet:server', this.conn.currentServer);

    _storage.default.setItem('wallet:multisig', !!this.multisig);
    ...
    if (this.seed) {
      ret = _wallet.default.executeGenerateWallet(this.seed, this.passphrase, pinCode, password, false, this.multisig);
    } else if (this.xpriv) {
      ret = _wallet.default.executeGenerateWalletFromXPriv(this.xpriv, pinCode, false, this.multisig);
    } else if (this.xpub) {
      ret = _wallet.default.executeGenerateWalletFromXPub(this.xpub, false, this.multisig);
    } else {
    ...
    const promise = new Promise((resolve, reject) => {
      _version.default.checkApiVersion().then(info => {
  ...
  async prepareCreateNewToken(name, symbol, amount, options = {}) {
    ...
    const ret = _tokens.default.generateCreateTokenData(mintAddress, name, symbol, amount, pin, newOptions);

    if (!ret.success) {
      return Promise.reject(ret);
    }

    return _helpers.default.createTxFromData(ret.preparedData, this.getNetworkObject());
  }
  /**
  ...
  selectAuthorityUtxo(tokenUid, filterUTXOs, options = {}) {
    ...
    for (const tx_id in walletData.historyTransactions) {
      ...
      for (const [index, output] of tx.outputs.entries()) {
        ...
        if (!_wallet.default.isAddressMine(output.decoded.address, walletData)) {
          continue;
  ...
  getMintAuthority(tokenUid, options = {}) {
    ...
    return this.selectAuthorityUtxo(tokenUid, _wallet.default.isMintOutput.bind(_wallet.default), newOptions);
  }
  /**
  ...
  getMeltAuthority(tokenUid, options = {}) {
    ...
    return this.selectAuthorityUtxo(tokenUid, _wallet.default.isMeltOutput.bind(_wallet.default), newOptions);
  }
  /**
  ...
  async prepareMintTokensData(tokenUid, amount, options = {}) {
    ...
    const mintAddress = newOptions.address || this.getCurrentAddress().address;
    const mintInput = this.selectAuthorityUtxo(tokenUid, _wallet.default.isMintOutput.bind(_wallet.default));

    if (mintInput.length === 0) {
      return {
        success: false,
        message: 'Don\'t have mint authority output available.'
      };
    }

    const ret = _tokens.default.generateMintData(mintInput[0], tokenUid, mintAddress, amount, null, pin, newOptions);
  ...
  async prepareMeltTokensData(tokenUid, amount, options = {}) {
    ...
    const meltInput = this.selectAuthorityUtxo(tokenUid, _wallet.default.isMeltOutput.bind(_wallet.default));

    if (meltInput.length === 0) {
      return Promise.reject({
        success: false,
        message: 'Don\'t have melt authority output available.'
      });
    } // Always create another melt authority output


    const ret = _tokens.default.generateMeltData(meltInput[0], tokenUid, amount, pin, newOptions.createAnotherMelt, {
  ...
  async prepareDelegateAuthorityData(tokenUid, type, destinationAddress, options = {}) {
    ...
    if (type === 'mint') {
      filterUtxos = _wallet.default.isMintOutput.bind(_wallet.default);
    } else if (type === 'melt') {
      filterUtxos = _wallet.default.isMeltOutput.bind(_wallet.default);
    } else {
    ...
    const ret = _tokens.default.generateDelegateAuthorityData(tx_id, index, address, tokenUid, destinationAddress, createAnother, type, pin);
  ...
  async prepareDestroyAuthorityData(tokenUid, type, count, options = {}) {
    ...
    const destroyInputs = this.selectAuthorityUtxo(tokenUid, filterUtxos, {
      many: true
    });
    ...
    if (destroyInputs.length < count) {
      return Promise.reject({
        ...
        errorData: {
          ...
          availableQuantity: destroyInputs.length
        }
      });
    }
    ...
    const ret = _tokens.default.generateDestroyAuthorityData(data, pin);
  ...
  getAuthorityUtxos(tokenUid, type) {
    ...
    return this.selectAuthorityUtxo(tokenUid, filterUtxos, {
      many: true
    });
  }
  ...
  getTokenData() {
    ...
    if (this.tokenUid === _constants.HATHOR_TOKEN_CONFIG.uid) {
      // Hathor token we don't get from the full node
      this.token = _constants.HATHOR_TOKEN_CONFIG;
    } else {
      ...
      _wallet2.default.getGeneralTokenInfo(this.tokenUid, response => {
        if (response.success) {
          this.token = {
            uid: this.tokenUid,
            name: response.name,
  ...
  isAddressMine(address) {
    return _wallet.default.isAddressMine(address);
  }
  /**
  ...
  getAddressIndex(address) {
    return _wallet.default.getAddressIndex(address);
  }
  /**
  ...
  async getTxBalance(tx, optionsParam = {}) {
    ...
    for (const txout of tx.outputs) {
      if (_wallet.default.isAuthorityOutput(txout)) {
    ...
    for (const txin of tx.inputs) {
      if (_wallet.default.isAuthorityOutput(txin)) {
        if (options.includeAuthorities) {
