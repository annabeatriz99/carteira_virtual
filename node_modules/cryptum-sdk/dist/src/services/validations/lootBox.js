...
module.exports.validateLootBoxDeploy = ({ protocol, wallet, name, symbol, royaltyRecipient, trustedForwarders, royaltyBps }) => {
    ...
    if (trustedForwarders && (!Array.isArray(trustedForwarders) || !trustedForwarders.length)) {
        throw new GenericException('Invalid parameter trustedForwarders, it should be an array with length larger than 0', 'InvalidTypeException');
    }
...
module.exports.validateLootBoxCreation = ({ protocol, contents, wallet, lootBoxAddress, lootBoxURI, openStartTimestamp, recipient, amountDistributedPerOpen, rewardUnits }) => {
    if (!wallet) {
        throw new InvalidException('Invalid wallet');
    }
    if (![Protocol.BSC, Protocol.CELO, Protocol.ETHEREUM, Protocol.AVAXCCHAIN, Protocol.POLYGON].includes(protocol)) {
    ...
    if (!Array.isArray(contents) || !contents.length) {
        throw new GenericException('Invalid parameter contents, it should be an array with length larger than 0', 'InvalidTypeException');
    }
    ...
    if (rewardUnits && (!Array.isArray(rewardUnits) || !contents.length)) {
        throw new GenericException('Invalid parameter rewardUnits, it should be an array with length larger than 0', 'InvalidTypeException');
    }
};
...
module.exports.validateApproveContent = ({ protocol, wallet, lootBoxAddress, tokenAddress, tokenId, tokenType, amount }) => {
    if (!wallet) {
        throw new InvalidException('Invalid wallet');
    }
    if (![Protocol.BSC, Protocol.CELO, Protocol.ETHEREUM, Protocol.AVAXCCHAIN, Protocol.POLYGON].includes(protocol)) {
