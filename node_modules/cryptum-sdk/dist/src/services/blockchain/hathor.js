...
module.exports.buildHathorTokenTransaction = async function ({ inputs, tokenName, tokenSymbol, address, changeAddress, mintAuthorityAddress, meltAuthorityAddress, amount, inputSum, tokenUid, type, nftData, testnet, }) {
    hathorLib.network.setNetwork(testnet ? 'testnet' : 'mainnet');
    let txData = {
        inputs: inputs.map((input) => ({ tx_id: input.txHash, index: input.index })),
        outputs: []
    ...
    const _dataToken = [TransactionType.HATHOR_TOKEN_MELT, TransactionType.HATHOR_NFT_MELT].includes(type)
        ? txData
