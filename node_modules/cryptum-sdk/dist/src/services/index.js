...
const handleRequestError = (error) => {
    if (error) {
        if (error.response) {
            ...
            const code = typeof error.response.data.error === 'string'
...
const makeRequest = async ({ method, url, params, headers, body, config }) => {
    try {
        const axios = new AxiosApi(config);
        const api = axios.getInstance();
        const response = await api({ method, url, params, headers: Object.assign(Object.assign({}, headers), mountHeaders(config.apiKey)), data: body });
