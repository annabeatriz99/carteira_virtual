...
const { buildSolanaTransferTransaction, deploySolanaToken, deploySolanaNFT, mintEdition, buildSolanaTokenBurnTransaction, updateMetaplexMetadata, buildSolanaCustomProgramInteraction, mintSolanaToken, updateAuctionAuthority, updateVaultAuthority, validateAuction, whitelistCreators, deploySolanaCollection, } = require('../../../services/blockchain/solana');
...
const { validateSolanaCollectionInput, validateSolanaNFTInput, validateSolanaTransferTransaction, validateSolanaDeployTransaction, validateSolanaDeployNFT, validateSolanaCustomProgramInput } = require('../../../services/validations/solana');
class Controller extends Interface {
     ...
     */
    async getFee({ type = null, from = null, amount = null, destination = null, contractAddress = null, contractAbi = null, method = null, params = null, protocol, contractName = null, source = null, feeCurrency = null, tokenType = null, numInputs = null, numOutputs = null }) {
        try {
            const data = {};
            if (type)
                data.type = type;
   ...
   */
    ...
    */
    ...
    async createBitcoinTransferTransaction(input) {
        ...
        if (wallet) {
            ...
            for (let i = 0; i < utxos.length; ++i) {
                inputs[i] = new Input(Object.assign(Object.assign({}, utxos[i]), { privateKey: wallet.privateKey }));
            }
        }
        else if (inputs) {
    ...
    async createHathorTokenTransactionFromWallet(input) {
        ...
        let { wallet, tokenName, tokenSymbol, type, tokenUid, mintAuthorityAddress, meltAuthorityAddress, amount, address, changeAddress, nftData, } = input;
        const protocol = Protocol.HATHOR;
        let inputSum = 0;
        let amountHTRUnit;
    ...
    async createHathorTokenTransactionFromUTXO(input) {
        ...
        let { inputs, tokenName, tokenSymbol, address, changeAddress, mintAuthorityAddress, meltAuthorityAddress, amount, tokenUid, type, } = input;
        let inputSum = 0;
        const protocol = Protocol.HATHOR;
        for (let i = 0; i < inputs.length; ++i) {
            const tx = await this.getTransactionByHash({ hash: inputs[i].txHash, protocol });
    ...
    async whitelistCreatorsTransaction(input) {
        const protocol = Protocol.SOLANA;
        const { blockhash } = await this.getBlock({ block: 'latest', protocol });
        const txHash = await whitelistCreators(Object.assign(Object.assign({}, input), { latestBlock: blockhash, config: this.config }));
        return new TransactionResponse({ hash: txHash });
    }
}
